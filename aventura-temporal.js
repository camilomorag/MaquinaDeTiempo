document.addEventListener("DOMContentLoaded", () => {
    ...
    // Dibujar interfaz
    function drawUI() {
      // Dibujar cuadro de diálogo si está abierto
      if (dialogOpen) {
        ctx.fillStyle = "rgba(0, 0, 0, 0.8)"
        ctx.fillRect(50, canvas.height - 100, canvas.width - 100, 80)
        ctx.strokeStyle = "#00FFFF"
        ctx.lineWidth = 2
        ctx.strokeRect(50, canvas.height - 100, canvas.width - 100, 80)
  
        ctx.fillStyle = "#FFFFFF"
        ctx.font = "16px Rajdhani"
        ctx.textAlign = "left"
        ctx.fillText(currentDialog, 70, canvas.height - 70)
        ctx.fillText("Presiona ESPACIO para continuar...", canvas.width - 300, canvas.height - 30)
      }
    }
        ...
        if (obj.era === currentEra && !obj.collected) {
          if (
            ...
            if (emptySlot !== -1) {
              ...
              // Mostrar mensaje
              showDialog(`Has encontrado: ${obj.name}`)
            }
          }
        }
      })
      ...
      portals.forEach((portal) => {
        if (portal.fromEra === currentEra) {
          if (
            ...
            showDialog(`Has viajado a: ${eras[currentEra]}`)
          }
        }
      })
    }
    ...
    function checkTimeMachineInteraction() {
      if (
        ...
        if (collectedCount === 5 && !timeMachine.repaired) {
          ...
          showDialog("¡Has reparado la máquina del tiempo! Ahora puedes viajar a cualquier época con seguridad.")
  
          // Victoria
          setTimeout(() => {
            endGame(true)
          }, 3000)
        } else if (!timeMachine.repaired) {
          showDialog(`La máquina del tiempo está averiada. Necesitas encontrar todas las piezas (${collectedCount}/5).`)
        }
      }
    }
    ...
    function startGame() {
      ...
      showDialog(
        "La máquina del tiempo se ha averiado. Debes encontrar las 5 piezas dispersas en diferentes épocas para repararla.",
      )
    }
    ...
    document.addEventListener("keydown", (e) => {
      ...
      // Si hay diálogo abierto, solo permitir cerrarlo
      if (dialogOpen) {
        if (e.code === "Space") {
          dialogOpen = false
        }
    ...
    // Iniciar el juego automáticamente después de 1 segundo
